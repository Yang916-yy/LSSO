"""Archived UEA-30 multivariate time-series classification experiment."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        nargs="*",
        default=(),
        help="optional odd temporal kernels, for example: 3 7 15",
    )
    ...
    parser.add_argument("--validation-folds", type=int, default=0)
    parser.add_argument("--validation-fold-index", type=int, default=0)
    parser.add_argument(
        "--train-full",
        action="store_true",
        help="train for exactly --epochs on the complete official training split",
    )
    parser.add_argument(
        "--test-evaluation", action=argparse.BooleanOptionalAction, default=True
    )
...
def main() -> None:
    ...
    elif args.validation_folds:
        train_indices, validation_indices = stratified_fold_indices(
            train_labels,
    ...
    encoder = SequenceValueEncoder(
        ...
        local_stem_kernels=tuple(args.local_stem_kernels),
    )
    ...
    train_classifier(
        ...
        metadata={
            ...
            "position_rank": args.position_rank,
            "pooling": args.pooling,
            "local_stem_kernels": list(args.local_stem_kernels),
