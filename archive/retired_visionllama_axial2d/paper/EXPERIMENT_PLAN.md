...
### 4.3 Scope boundary: UEA-30

UEA-30 is excluded from the formal auxiliary suite. Its many very small
training splits are poorly matched to the current wide RRLSSO encoder: the
completed 30-task probe produced unstable model selection and a 67.67% mean,
while controlled local-stem experiments did not outperform the MHA anchor on
the three investigated difficult datasets. The implementation and results are
retained as a reproducibility artifact and negative scope analysis, but they
do not enter the paper's generality claim, primary metrics, or model ranking.
...
- Pre-register primary metrics: ImageNet top-1, COCO box AP, ADE20K mIoU,
  GenomicBenchmarks eight-task mean rank, and LRA four-task mean accuracy.
- Report all seeds, not only the best checkpoint. Select checkpoints using a
  validation metric fixed before training.
- Use mean and standard deviation for seed repeats and paired per-task
  comparisons for the formal auxiliary suites.
...
VisionLLaMA-B -> COCO/ADE20K -> GenomicBenchmarks -> LRA. A failed
