...
def test_rank_rotary_preserves_norm_and_relative_kernel() -> None:
    torch.manual_seed(0)
    B, H, N, r = 2, 3, 9, 8
    U = torch.randn(B, H, N, r, dtype=torch.float64)
    positions = torch.arange(N, dtype=torch.float64)
    U_rotary = apply_rank_rotary(U, positions)

    torch.testing.assert_close(U_rotary.norm(dim=-1), U.norm(dim=-1), atol=1e-12, rtol=1e-12)

    shifted = apply_rank_rotary(U, positions + 17)
    kernel = torch.matmul(U_rotary, U_rotary.transpose(-2, -1))
...
def test_solve_state_read_matches_bidirectional_rrlsso() -> None:
    ...
    U_rotary = apply_rank_rotary(U)
    cache = update_solve_state(None, U_rotary, C)
    y_cache = read_solve_state(U_rotary, C, cache, mu, gamma)
    y_ref = lsso(U_rotary, C, mu, gamma)
    torch.testing.assert_close(y_cache, y_ref, atol=1e-10, rtol=1e-10)


if __name__ == "__main__":
    test_rank_rotary_preserves_norm_and_relative_kernel()
