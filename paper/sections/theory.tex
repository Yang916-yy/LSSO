...
These are conditional bounds for the linear map at fixed $U$; because both $U$ and $C$ depend on $X$, they are not by themselves a global Lipschitz bound for the complete nonlinear layer. Row-wise RMS normalization controls individual relation-vector scale, while \cref{eq:length_norm} controls the accumulation scale of $U^\top U$. Neither should be confused with a hard dimension-free bound on $\lVert U\rVert_2$.
