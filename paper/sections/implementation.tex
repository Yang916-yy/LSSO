...
The operator studied here is bidirectional: a single solve uses statistics from all valid tokens. We do not derive or claim a causal decoding recurrence. We also distinguish the mathematical inverse from a generic Sylvester equation solver: although the module name is retained, the implemented primitive is a positive-shifted low-rank linear system with multiple right-hand sides. This narrower description exposes the exact assumptions used by the theory and implementation.
