...
the layer.  At fixed compact rank, both arithmetic and activation storage are
linear in sequence length.  We provide an algebraically equivalent native CUDA
path with an analytic backward that reuses the compact factors.  In a matched
eight-task
GenomicBenchmarks comparison against multi-head attention and three
representative linear-attention baselines, LSSO leads on seven tasks and raises
unweighted macro accuracy from the strongest alternative's $75.57$ to $78.53$.
Across four Long Range Arena tasks with sequences up to $4096$ positions, LSSO
attains the highest four-task average among the displayed general-purpose
global mixers.
\end{abstract}
...
\begin{enumerate}
    ...
    in time and activation storage with sequence length, and removes inner
    learning rates, update counts, and fixed-point iterations from the forward
    pass.
    ...
    matched three-seed comparison across eight genomic tasks, LSSO leads MHA,
    Linear Transformer, Performer, and cosFormer on seven tasks and exceeds the
    strongest alternative's macro accuracy by $2.96$ points.  A four-task LRA
    panel spans sequences up to length $4096$.  An algebraically equivalent
    native CUDA forward and analytic backward reaches up to $2.30\times$ the
    measured Flash-SDPA MHA training throughput without forming a token--token
    map.
\end{enumerate}
...
Table~\ref{tab:genomic-results} shows that LSSO is highest on seven of eight
tasks.  Its $78.53$ macro average exceeds cosFormer, the strongest alternative
...
LSSO and $7.92\times$ for MHA, matching their linear and quadratic mixer
arithmetic.
...
LSSO is currently bidirectional: its compact statistic summarizes the full
valid sequence before solving the equilibrium.  It therefore does not support
causal masking or autoregressive decoding.  Prefix-conditioned solved states
that preserve the contraction certificate and linear scaling are the principal
open extension.

The reference implementation supports workload-specific ranks; current native
packages target $r\in\{16,32,48,64\}$.  Their measured gains leave room for
shape- and architecture-specific autotuning.  Evaluation spans genomic
classification and four LRA tasks on SM120; vision and broader hardware
