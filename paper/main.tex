...
\begin{enumerate}
    ...
    \item \textbf{Complete and certifiable compact geometry.}  We cover the
    full open real matrix ball in rank space, including nonsymmetric and
    nonnormal contractions.  We derive an exact compact certificate for every
    realized token-space gain and a monotonicity margin that jointly bounds the
    solved state, contextual sensitivity, implicit adjoint, and flow rate.
...
compact-state line by using the compressed statistic to generate an accretive
system.  Its equilibrium is solved directly, and the resulting contraction is
applied without constructing a dense token matrix.
...
\paragraph{Structured sequence models and certifiable equilibria.}
...
structure \cite{winston2020mondeq}.  LSSO combines global noncausal
conditioning with accretive/resolvent geometry \cite{bauschke2017convex}.  It
provides an exact gain certificate for each realized token mixer, while skew
coordinates retain nonsymmetric and nonnormal contractions.  This offers a
directly measurable operator guarantee in a setting where ordinary
self-attention need not be globally Lipschitz \cite{kim2021lipschitz}.
