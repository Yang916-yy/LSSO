...
LSSO builds on two complementary advances.  XCiT shows that cross-covariance
statistics can define an efficient, input-dependent global operator shared
across tokens \cite{ali2021xcit}.  Direct Cayley parameterization provides a
complete differentiable map from free coordinates to static
Lipschitz-bounded network weights \cite{wang2023direct}.  LSSO takes the next
step: it turns static contractive weights into a compact Cayley token mixer
generated anew from every input, solved exactly, and certified on the realized
sample.
...
\begin{enumerate}
    ...
    nonsymmetric and nonnormal contractions.
    \item \textbf{Exact per-instance certification.}  For every realized token
    mixer, we derive its exact gain $q(X)<1$ entirely in compact space.  A
    complementary sample-dependent monotonicity margin $\mu>1$ certifies
    the equilibrium, contextual sensitivity, implicit adjoint, and
    continuous-time convergence rate.  Both certificates require only
    matrices of size at most $r\times r$ and are directly observable in trained
    networks.
...
operator mathematics.  Although the native path provides complete forward and
analytic backward execution for the default operator, its kernel optimization
is not yet as comprehensive as FlashAttention's across workload shapes and GPU
generations.  This systems gap is also concrete headroom: shape-specific
schedules and autotuning can yield further wall-clock gains without changing
the operator.  The reported measurements therefore characterize the current
implementation, not a performance ceiling for LSSO.  The present empirical
evidence covers genomic
