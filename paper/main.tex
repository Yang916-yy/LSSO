...
The reference implementation permits workload-specific ranks, whereas the
native CUDA path is optimized and distributed only for the schedules in
Appendix~\ref{app:cuda}.  Its complete forward and analytic backward paths are
not yet as comprehensively optimized across workload shapes and GPU generations
as FlashAttention, leaving concrete headroom for specialized schedules and
autotuning.  The reported wall-clock measurements therefore characterize the
current implementation rather than LSSO's performance ceiling.  Empirical
evaluation currently covers genomic classification, four LRA tasks, and one
recent NVIDIA architecture; vision and broader hardware remain open.
